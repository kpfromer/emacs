...
;;          ;; Cache this temp dir for faster perf
;;          (temp-dir (make-temp-file "thumbnails-" t))
;;             (thumbnail-file (concat temp-dir (file-name-nondirectory (file-name-sans-extension video-file)) ".png")))
;;          ;; (thumbnail-file (concat (file-name-sans-extension video-file) ".png")))
;;     (generate-video-thumbnail video-file thumbnail-file)
...
;;    If `ranger-show-literal' is set, show literally instead of actual buffer."
;;   (if (and (not (file-directory-p entry-name))
;;            (video-file-p entry-name))
;;       (ranger-preview-video-thumbnail entry-name)
;;     ;; Original preview logic for non-video files goes here...
;;     ))
